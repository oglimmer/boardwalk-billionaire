  ...
  <div class="modal-overlay" :class="{ show: store.modal.visible }">
    <div class="modal" :class="{ wide: isWide }">
      ...
      <template v-if="store.modal.type === 'buy'">
        ...
        <div class="btn-group">
          <button class="btn-yes" :disabled="!canAffordBuy" @click="store.buyProperty()">Buy - ${{ buySpace.price }}</button>
      ...
      <template v-if="store.modal.type === 'win'">
        ...
        <div class="btn-group">
          <button class="btn-yes" @click="handlePlayAgain()">Play Again</button>
        </div>
      </template>
      ...
      <template v-if="store.modal.type === 'card'">
        ...
        <div class="btn-group">
          <button class="btn-opt" @click="store.acknowledgeCard()">OK</button>
        </div>
      </template>
      ...
      <template v-if="store.modal.type === 'tradeSelect'">
        ...
        <div class="btn-group vertical">
          <template v-for="(pl, i) in store.players" :key="i">
            <button
              v-if="i !== store.yourPlayerIndex && !pl.isBankrupt && (tradePartnerProps(i).length > 0 || tradeMyProps.length > 0)"
          ...
          <div class="trade-col">
            ...
            <div class="trade-cash">
              + Cash: $<input
                ...
                min="0"
                :max="store.players[store.yourPlayerIndex].money"
                step="10"
                class="cash-input"
...
const tradeMyProps = computed(() => store.getTradeableProperties(store.yourPlayerIndex))
...
const hasTradePartner = computed(() => {
  return store.players.some((pl, i) => {
    if (i === store.yourPlayerIndex || pl.isBankrupt) return false
