...
DEFAULT_FRONTEND_DEPLOYMENT="boardwalk-billionaire-frontend"
DEFAULT_BACKEND_DEPLOYMENT="boardwalk-billionaire-backend"
...
Build, deploy, and release Boardwalk Billionaire application components.
...
parse_args() {
    ...
    if [[ ${#REGISTRIES[@]} -gt 0 ]]; then
        ...
        for registry in "${REGISTRIES[@]}"; do
            FRONTEND_IMAGES+=("$registry/boardwalk-billionaire-fe")
            BACKEND_IMAGES+=("$registry/boardwalk-billionaire-be")
        done
    else
        # Fallback to defaults if no registries specified
        FRONTEND_IMAGES=("${DEFAULT_REGISTRIES[0]}/boardwalk-billionaire-fe")
        BACKEND_IMAGES=("${DEFAULT_REGISTRIES[0]}/boardwalk-billionaire-be")
    fi
...
execute_build() {
    ...
    # Build frontend
    if [[ "$BUILD_FRONTEND" == true ]]; then
        build_image "frontend" "-f client/Dockerfile client" "${FRONTEND_IMAGES[@]}"
    fi

    # Build backend
    if [[ "$BUILD_BACKEND" == true ]]; then
        build_image "backend" "-f server/Dockerfile server" "${BACKEND_IMAGES[@]}"
    fi
